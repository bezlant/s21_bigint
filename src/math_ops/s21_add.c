...
int s21_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
    ...
    if (get_sign(value_1) == 0 && get_sign(value_2) == 0) {
        ...
        s21_sub(value_1, value_2, code);
    } else if (get_sign(value_1) == 1 && get_sign(value_2) == 0) {
        s21_sub(value_2, value_1, code);
...
void handle_exponent_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result, int *code) {
    ...
    s21_decimal value_2_check_overflow = value_2;

    for (int i = 0; i < abs(exp_v1 - exp_v2); i++) {
        value_2_check_overflow = binary_multiplication(value_2, power_of_ten, code);
        if (*code == 1) break;
